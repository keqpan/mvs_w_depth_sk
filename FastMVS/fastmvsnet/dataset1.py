...
from fastmvsnet.utils.preprocess import mask_depth_image, norm_image, norm_depth, norm2_depth, scale_dtu_input, crop_dtu_input
...
class DTU_Train_Val_Set(Dataset):
    ...
    def _load_dataset(self, dataset, lighting_set):
        ...
        for ind in dataset:
            ...
            input_depth_folder = osp.join(self.root_dir, "input_depth/scan{}_train".format(ind))
            ...
            for lighting_ind in lighting_set:
                ...
                for p in range(0, int(self.cluster_list[0])):
                    ...
                    ref_input_depth_path = osp.join(input_depth_folder, "depth_map_{:04d}.pfm".format(ref_index))

                    view_image_paths.append(ref_image_path)
                    view_cam_paths.append(ref_cam_path)
                    view_depth_paths.append(ref_depth_path)
                    view_input_depth_paths.append(ref_input_depth_path)
    ...
    def __getitem__(self, index):
        ...
        for input_depth_path in paths["view_input_depth_paths"]:
            input_depth_image = io.load_pfm(input_depth_path)[0]
            input_depth_images.append(input_depth_image)
        ...
        # mask out-of-range input_depth pixels (in a relaxed range)
        ref_input_depth = input_depth_images[0]
        input_depth_start = cams[0][1, 3, 0] + cams[0][1, 3, 1]
        input_depth_end = cams[0][1, 3, 0] + (self.num_virtual_plane - 2) * cams[0][1, 3, 1]
        ref_input_depth = mask_depth_image(ref_input_depth, input_depth_start, input_depth_end)

        depth_list = np.stack(depth_images, axis=0)
        input_depth_list = np.stack(input_depth_images, axis=0)
        #print("depth_images", depth_images[0].shape)
        #print("input_depth_images", input_depth_images[0].shape)
        ...
        ref_input_depth = torch.tensor(ref_input_depth).permute(2, 0, 1).type(torch.float)
        input_depth_list = torch.tensor(input_depth_list).unsqueeze(-1).permute(0, 3, 1, 2).type(torch.float)
        input_depth_list = input_depth_list * (input_depth_list > input_depth_start).float() * (input_depth_list < input_depth_end).float()
...
class DTU_Test_Set(Dataset):
    ...
    def __init__(self, root_dir, dataset_name,
        ...
        self.input_depth_folder = input_depth_folder
...
#         ref_input_depth = input_depth_images[0].copy()
        ...
        for i, depth in enumerate(cropped_input_depths):
            cropped_input_depths[i] = norm_image(depth)
        ...
        input_depth_list = torch.tensor(input_depth_list).unsqueeze(-1).permute(0, 3, 1, 2).float()
